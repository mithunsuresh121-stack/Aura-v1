...
echo "============================================"
echo "  AURA — Local AI Agent Setup"
echo "============================================"
echo ""
...
$PYTHON -m pip install -r "$SCRIPT_DIR/aura-train/requirements.txt" -q
...
mkdir -p "$SCRIPT_DIR/aura-train/data"

# --- Download pre-trained model (if available) ---
echo ""
echo "--- Model setup ---"
MODEL_DIR="$SCRIPT_DIR/aura-train/checkpoints"
mkdir -p "$MODEL_DIR"

if [ ! -f "$MODEL_DIR/final.pt" ]; then
    echo "No pre-trained model found. Run 'bash download_model.sh' to download a checkpoint,"
    echo "or train your own with: cd aura-train && $PYTHON train_v2.py"
echo "  Or use the pretrained server: bash aura-train/serve_pretrained.sh"
else
    echo "[✓] Model checkpoint found: $MODEL_DIR/final.pt"
fi

# --- Train or download tokenizer ---
if [ ! -f "$SCRIPT_DIR/aura-train/data/tokenizer.json" ]; then
...
tok.save('$SCRIPT_DIR/aura-train/data/tokenizer.json')
...
echo "  cd $(pwd) && $PYTHON aura-core/src/server/main.py"
echo ""
echo "Start the frontend (dev mode):"
echo "  cd aura-app && npm run dev"
echo ""
