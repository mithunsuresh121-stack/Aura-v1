...
import sys
sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "aura-train"))
sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "aura-rag"))
...
sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "aura-core" / "src"))
...
async def lifespan(app: FastAPI):
    ...
    ckpt = os.environ.get("AURA_CHECKPOINT")
    data_dir = os.environ.get("AURA_DATA_DIR", str(Path(__file__).resolve().parents[3] / "aura-train" / "data"))
    user_dir = os.environ.get("AURA_USER_DIR", str(Path(__file__).resolve().parents[3] / "aura-core" / "users"))
    user_store = UserStore(user_dir)
    quantize = os.environ.get("AURA_QUANTIZE", "none")
    load_model(ckpt, data_dir, quantize)
    hypernet_ckpt = os.environ.get("AURA_HYPERNET_CHECKPOINT")
    ...
    prompt_dir = os.environ.get("AURA_PROMPT_DIR", "")
    ...
    grants_path = os.environ.get("AURA_GRANTS_PATH", str(Path(__file__).resolve().parent.parent / "grants.json"))
    grant_store = GrantStore(grants_path if Path(grants_path).exists() else None)
    build_engine = BuildEngine()

    # Model registry — supports local + Ollama/OpenAI/Gemini backends
    global model_registry
    models_config = os.environ.get("AURA_MODELS_CONFIG", str(Path(__file__).resolve().parent.parent.parent / "models.json"))
    model_registry = ModelRegistry(models_config)
    if model is not None and tokenizer is not None:
        model_registry.register_local("aura-core", model, tokenizer, device)
    logger.info(f"[models] registered: {model_registry.available}, default: {model_registry.default}")
    ...
    logger.info("[capabilities] terminal, computer, video, grants, build, models ready")

    if hypernetwork is not None and model is not None and improvement_engine is None:
        imp_dir = os.environ.get("AURA_IMPROVEMENT_DIR", "")
        if imp_dir:
    ...
    if hypernetwork is not None and model is not None and agent is None:
        kb_path = os.environ.get("AURA_KNOWLEDGE_BASE", "")
        ...
        identity_path = os.environ.get("AURA_IDENTITY_PATH", "")
        agent = Agent(
            base_model=model,
            hypernetwork=hypernetwork,
            lora_rank=hypernetwork_config.get("lora_rank", 8),
            ...
            memory_path=os.environ.get("AURA_MEMORY_PATH", "memory.sqlite"),
        )
        logger.info(f"[agent] ready, identity v{agent.identity.version}")

    yield

    # Shutdown: close terminal sessions
    terminal_agent.close_all()
...
# Include capabilities (computer, terminal, video, MCP, orchestration)
...
# Model registry (local + Ollama/OpenAI/Gemini)
...
class ChatCompletionRequest(BaseModel):
    model: str = Field(default="aura-core", description="Model ID")
    ...
    if tok_type == "gpt2":
        ...
        if not tok_path.exists():
            raise RuntimeError(
                ...
                "or set AURA_DATA_DIR to a directory containing tokenizer.json"
            )
        from tokenizers import Tokenizer as HFTokenizer
...
async def save_identity():
    ...
    if agent is None:
        raise HTTPException(503, "Agent not loaded")
    path = os.environ.get("AURA_IDENTITY_PATH", "identity.pt")
...
async def list_models():
    if model_registry is not None:
        return ModelList(data=[
            ModelInfo(id=m, created=int(time.time())) for m in model_registry.available
        ])
    return ModelList(data=[
        ModelInfo(id="aura-core", created=int(time.time())),
    ])
...
async def chat_completions(req: ChatCompletionRequest, raw_request: Request):
    ...
    # Route external models through the registry
    if model_registry is not None and req.model != "aura-core":
        try:
            backend = model_registry.get(req.model)
        except ValueError:
            raise HTTPException(400, f"Model '{req.model}' not available. Options: {model_registry.available}")
        ...
        if req.stream:
            async def _external_stream():
                ...
                async for chunk in backend.generate_stream(
                    ...
                    yield json.dumps({"choices": [{"delta": {"content": chunk}, "index": 0}]})
                yield json.dumps({"choices": [{"delta": {}, "finish_reason": "stop", "index": 0}]})
                yield "data: [DONE]"
            return EventSourceResponse(_external_stream())
        ...
        return {
            ...
            "usage": {
                ...
                "total_tokens": sum(len(m.content.split()) for m in req.messages) + completion_tokens,
            },
        }
    ...
    try:
        ...
        # Collect self-improvement data from accumulated logits
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", type=str, default=str(Path(__file__).resolve().parents[3] / "aura-train" / "data"))
    ...
    os.environ["AURA_CHECKPOINT"] = args.checkpoint or ""
    os.environ["AURA_HYPERNET_CHECKPOINT"] = args.hypernet_checkpoint or ""
    os.environ["AURA_QUANTIZE"] = args.quantize
    os.environ["AURA_IMPROVEMENT_DIR"] = args.improvement_dir
    os.environ["AURA_KNOWLEDGE_BASE"] = ""
    os.environ["AURA_PROMPT_DIR"] = args.prompt_dir
    os.environ["AURA_IDENTITY_PATH"] = args.identity_path or ""
    os.environ["AURA_MEMORY_PATH"] = args.memory_path or "memory.sqlite"
    os.environ["AURA_DATA_DIR"] = args.data_dir
